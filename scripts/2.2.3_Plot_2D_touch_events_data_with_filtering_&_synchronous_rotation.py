...
from tkinter.ttk import Checkbutton, Scale, Button
...
def create_plot(event_types):
    ...
    legend = ax.legend(unique_labels.values(), unique_labels.keys(), title="Legend", bbox_to_anchor=(1.05, 1), loc='upper left', fontsize='small')  # Move legend to the side
...
# Function to update global rotation values
def update_rotation():
    global current_elevation, current_azimuth
    current_elevation = elevation_slider.get()
    current_azimuth = azimuth_slider.get()
...
# Create sliders to control rotation
elevation_label = tk.Label(root, text="Elevation")
elevation_label.pack(anchor='w')
elevation_slider = Scale(root, from_=0, to=180, orient='horizontal')
elevation_slider.pack(anchor='w')

azimuth_label = tk.Label(root, text="Azimuth")
azimuth_label.pack(anchor='w')
azimuth_slider = Scale(root, from_=0, to=360, orient='horizontal')
azimuth_slider.pack(anchor='w')

# Function to handle create plot button click
def create_plot_button_click():
    event_types = []
    if var_36.get():
    ...
    create_plot(event_types)

# Create button to create plot
create_plot_button = Button(root, text="Create Plot", command=create_plot_button_click)
create_plot_button.pack()

# Create button to update rotation
update_rotation_button = Button(root, text="Update Rotation", command=update_rotation)
update_rotation_button.pack()
