...
root.geometry("350x300")  # Adjusted size for the tkinter window
...
def create_plot(event_types, selected_colors):
...
# Checkboxes and their labels for event types
...
# Variables to hold color checkbox states
color_vars = {color_name: IntVar(value=0) for color_name in unique_colors}

# Checkboxes for colors
color_checkboxes = []
for color_name in unique_colors:
    checkbox = Checkbutton(root, text=color_name, variable=color_vars[color_name])
    checkbox.pack(anchor='w')
    color_checkboxes.append(checkbox)

# Function to trigger plotting with selected event types and colors
def on_plot_button_click():
    ...
    selected_colors = [color_name for color_name, var in color_vars.items() if var.get() == 1]
    if not selected_colors:
        selected_colors = unique_colors  # Default to all colors if none are selected

    create_plot(selected_events, selected_colors)
